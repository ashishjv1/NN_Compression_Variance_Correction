...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='cuda:2', help='Device to train')
    ...
    if args.resume_model:
        ...
        net = model_loader_cifar.load(args.model)
        net.load_state_dict(checkpoint['state_dict'])
        start_epoch = checkpoint['epoch'] + 1
    else:
        model = model_loader_cifar.load(args.model)
#         print(net)
        names_to_compress = [module_name for module_name, module in model.named_modules()
                              if isinstance(module, nn.Conv2d)]
        init_params(model)
#         print(model)
        CNET, net = compress_layers(model,  key='tucker2', lr=0.0001, penalty=0.4, n_iter=1100)
#         print(net)
        torch.save(net, 'save_models/' + str(args.model) + 'full' + '.pth')
        net = net.to(device)
